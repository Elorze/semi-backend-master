class HomeController < ApplicationController
  ...
  def send_sms
    ...
    if ENV["SMS_ENABLED"] == "ENABLED"
      ...
      Rails.logger.info("[SMS MOCK] phone: #{phone}, code: #{code} (未发送，仅开发测试)")
    end
  ...
  def send_email
    ...
    if ENV["EMAIL_ENABLED"] == "ENABLED"
      mailer = SigninMailer.with(code: code, recipient: email).signin
      mailer.deliver_now!
      Rails.logger.info("Email sent to: #{email}, code: #{code}")
    else
      Rails.logger.info("[EMAIL MOCK] email: #{email}, code: #{code} (未发送，仅开发测试)")
    end
